        ...
        for (x,y) in group:
            for (s,t) in ((x-1,y),(x+1,y),(x,y-1),(x,y+1)):
                if (s > 0 and s < (im2.size[0]) and t > 0 and t < (im2.size[1])):
                    if (im2.getpixel((s,t))[0] == 117 or im2.getpixel((s,t))[0] == 255):
                        totWhi += 1
        if (totWhi > 1):
            for (x,y) in group:
...
def growDis(ims, randPS, corners, dis):
    for (rx,ry) in randPS:
        lineCoords = []
...
for USELESS in range(0,10):
    ...
    cimage = ColorWhitelist(cimage, ((140, 0), (112, 0)))
    cimage = pixGroupRemove(cimage, 10)
    ...
    for (xt,yt) in trips:
        for (ut,it) in ((xt+1,yt),(xt-1,yt),(xt,yt+1),(xt,yt-1)):
            im.putpixel((xt,yt), (0,255))
            im.putpixel((ut,it), (0, 255))
