...
def getLetter(im):
    ...
    table = ['2', '3', '4', '5', '6', '7', '8', '9', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'K', 'L', 'M', 'N', 'P', 'R', 'T', 'U', 'W', 'Y', 'Z', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'k', 'l', 'm', 'n', 'p', 'r', 's', 't', 'u', 'v', 'w', 'y', 'z']
    letter = ''
    for ret in range(0,len(output)):
        if (output[ret] > 0):
            letter = table[ret]
...
ann.create_from_file("network.net")
...
for loops in range(0,20):
    ...
    for tups in segments:
        ...
        image = preimage.resize((30,30),Image.NEAREST)
